...
// to shop (see src/app/admin/layout.tsx and
// api/lib/adminAuth.js#requireAdmin, which re-checks the role on every request).
...
// Every call into the backend's /admin/* API needs both the shared
...
// Forwards a raw file upload to the backend. Separate from
