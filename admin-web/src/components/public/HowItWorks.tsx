...
// Same four steps, same wording, as the mobile app's ShoppingGuideModal -
// the cashback timeline has to read identically on web and in the app or the
// "when do I get my money" expectation drifts between the two.
const STEPS = [
  {
    icon: BagIcon,
    title: "Bấm vào sản phẩm",
    description:
      "Rewally sẽ chuyển bạn sang Shopee. Hãy đặt hàng ngay trong phiên vừa mở để đơn được ghi nhận hoàn tiền.",
  },
  {
    icon: ClockIcon,
...
export default function HowItWorks() {
  return (
    <section className="mt-8 rounded-3xl bg-[var(--surface-secondary)] px-5 py-9 sm:px-10">
      <div className="text-center">
        <h2 className="text-xl font-extrabold text-[var(--foreground)] sm:text-2xl">Mua sắm hoàn tiền thế nào?</h2>
      ...
      <ol className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {STEPS.map(({ icon: Icon, title, description }, index) => (
          <li key={title} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5">
      ...
      <div className="mt-7 text-center">
        <Link href="/guide" className="text-sm font-bold text-[var(--accent)] hover:underline">
